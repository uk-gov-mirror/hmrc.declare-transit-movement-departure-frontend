...
class UsingSameCommercialReferenceController @Inject()(
  ...
  formProvider: UsingSameCommercialReferenceFormProvider,
    ...
    implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(UsingSameCommercialReferencePage(itemIndex), value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(UsingSameCommercialReferencePage(itemIndex), mode, updatedAnswers))
        )
  }
}
