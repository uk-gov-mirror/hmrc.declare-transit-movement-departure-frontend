...
class ConfirmRemoveOfficeOfTransitController @Inject()(
  ...
  def onPageLoad(lrn: LocalReferenceNumber, mode: Mode): Action[AnyContent] = (identify andThen getData(lrn) andThen requireData).async {
    implicit request =>
      ...
      renderer.render("confirmRemoveOfficeOfTransit.njk", json).map(Ok(_))
  }
    ...
    implicit request =>
      ...
      form.bindFromRequest().fold(
        formWithErrors => {
          ...
          renderer.render("confirmRemoveOfficeOfTransit.njk", json).map(BadRequest(_))
        },
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(ConfirmRemoveOfficeOfTransitPage, value))
